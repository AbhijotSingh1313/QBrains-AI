...
class DisplayedCircuit {
    ...
    paint(painter, hand, stats, forTooltip=false, showWires=true) {
        let scrollX = forTooltip ? 0 : (DisplayedCircuit.scrollX || 0);

        painter.ctx.save();
        painter.ctx.beginPath();
        painter.ctx.rect(0, this.top, painter.canvas.width, this.desiredHeight());
        painter.ctx.clip();
        ...
        if (showWires) {
            this._drawWires(painter, !forTooltip, hand, scrollX);
        }

        for (let col = 0; col < this.circuitDefinition.columns.length; col++) {
            this._drawColumn(painter, this.circuitDefinition.columns[col], col, hand, stats, scrollX);
        }
     ...
     * @private
     */
    _drawWires(painter, showLabels, hand, scrollX=0) {
        ...
        if (showLabels) {
            for (let row = 0; row < drawnWireCount; row++) {
                ...
                let shiftedRect = rect.shiftedBy(-scrollX, 0);
                if (shiftedRect.right() > 0 && shiftedRect.x < painter.canvas.width) {
                    painter.noteTouchBlocker({rect: shiftedRect, cursor: 'pointer'});
                }
        ...
        painter.ctx.save();
        let maxWireX = Math.max(painter.canvas.width + scrollX, this.desiredWidth());
        for (let row = 0; row < drawnWireCount; row++) {
            ...
            painter.trace(trace => {
                ...
                for (let col = 0;
                        showLabels ? lastX < maxWireX : col <= this.circuitDefinition.columns.length;
                        col++) {
    ...
    _highlightStatusAt(col, row, focusPosPts) {
        if (this._highlightedSlot !== undefined) {
            let matches = false;
            let resizeStyle = false;
            if (Array.isArray(this._highlightedSlot)) {
                matches = this._highlightedSlot.some(s => s && s.col === col && (s.row === undefined || s.row === row));
            } else if (this._highlightedSlot.col === col && (this._highlightedSlot.row === undefined || this._highlightedSlot.row === row)) {
                matches = true;
                resizeStyle = !!this._highlightedSlot.resizeStyle;
            }
            if (matches) {
                return {
                    isResizeShowing: true,
                    isResizeHighlighted: resizeStyle,
                    isHighlighted: !resizeStyle
                };
            }
        }
     ...
     * @private
     */
    _drawColumn(painter, gateColumn, col, hand, stats, scrollX=0) {
        ...
        for (let row = 0; row < this.circuitDefinition.numWires; row++) {
            ...
            let shiftedGateRect = gateRect.shiftedBy(-scrollX, 0);
            if (shiftedGateRect.right() > 0 && shiftedGateRect.x < painter.canvas.width) {
                painter.noteTouchBlocker({rect: shiftedGateRect, cursor: 'pointer'});
                if (gate.canChangeInSize()) {
                    painter.noteTouchBlocker({rect: GatePainting.rectForResizeTab(gateRect).shiftedBy(-scrollX, 0), cursor: 'ns-resize'});
                }
            }
    ...
    _drawColumnDragHighlight(painter, col) {
        ...
        if (this._highlightedSlot !== undefined) {
            if (Array.isArray(this._highlightedSlot)) {
                isColMatch = this._highlightedSlot.some(s => s && s.col === col && s.row === undefined);
            } else if (this._highlightedSlot.col === col && this._highlightedSlot.row === undefined) {
                isColMatch = true;
            }
        }
        if (isColMatch) {
    ...
    withHighlightedSlot(slot) {
        return this._withHighlightedSlot(slot);
    }
