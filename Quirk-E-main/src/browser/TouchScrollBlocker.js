...
class TouchScrollBlocker {
    ...
    setBlockers(desiredBlockers, overrideCursorStyle) {
        ...
        // Positioning.
        let maxW = this._parentElement.clientWidth || 0;
        for (let i = 0; i < desiredBlockers.length; i++) {
            ...
            let clampedX = desiredArea.x;
            let clampedW = desiredArea.w;
            if (maxW > 0) {
                clampedX = Math.max(0, desiredArea.x);
                let clampedRight = Math.min(maxW, desiredArea.right());
                clampedW = Math.max(0, clampedRight - clampedX);
            }
