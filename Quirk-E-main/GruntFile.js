...
module.exports = function(grunt) {
    ...
    grunt.registerTask('inject-js-into-html', function(htmlSrc, jsSrc, dst) {
        ...
        var tutorPart = grunt.file.read('html/tutor.partial.html');
        var output = html;
        output = output.split("<!-- INCLUDE SOURCE PART -->").join(js);
        output = output.split("<!-- INCLUDE QUANTUM CIRCUIT -->").join(quantumCircuit);
        output = output.split("<!-- INCLUDE INSPECTOR PART -->").join(inspector);
        ...
        output = output.split("<!-- INCLUDE TUTOR PART -->").join(tutorPart);
    ...
    grunt.registerTask('build-debug', [
        ...
        'bootstrap-get-packages:src/main.js:out/tmp/traceur/bootstrap_post_src/run_main.js',
        'concat:concat-traceur-src',
        'inject-js-into-html:html/quirk.template.html:out/tmp/concatenated-src.js:out/index.html',
        'clean:clean-tmp'
    ]);
